...
def show_app():
    ...
    # Inicializar el estado de la ventana actual
    if "pagina_actual" not in st.session_state:
        st.session_state.pagina_actual = 1  # Comenzar en la ventana 1
    ...
    # Ventana 2: Mostrar Combos Recomendados
        ...
        if 'productos_seleccionados' in st.session_state and st.session_state.productos_seleccionados:
            if "modelo_ejecutado" not in st.session_state:
                st.session_state.modelo_ejecutado = False  # Control para ejecutar el modelo    
            ...
            # Convertir "Margen Combo" a valores numéricos y crear barra visual
            def generar_barra_visual(valor, longitud=20):
                num_I = int((valor / 100) * longitud)  # Calcular la cantidad de 'I'
            ...
            # Mostrar la barra superior (multiselect, botón guardar, navegación)
            ...
            with col2:
                ...
                with col2_1:
                    if st.button("Guardar Selección", use_container_width=True, type="secondary"):
                        ...
                        # Mostrar mensaje de éxito en col2_2
                        with col2_2:
                            st.success("¡Selección guardada!")
            with col4:
                st.button("Anterior", on_click=retroceder_pagina, use_container_width=True, type="primary")  # Cambia esta función según tu lógica
            with col5:
                st.button("Siguiente", on_click=avanzar_pagina, use_container_width=True, type="primary")  # Cambia esta función según tu lógica

            # Función para resaltar filas seleccionadas
            ...
            # Crear y mostrar la tabla estilizada con HTML
            styled_df = (
                df_combos.style
                .apply(resaltar_filas, axis=1)  # Resaltar filas seleccionadas
                .set_table_styles([
                    {"selector": "table", "props": [("border-collapse", "collapse"), ("margin", "auto")]},  # Centrar la tabla
                    {"selector": "th", "props": [
                        ("text-align", "center"), 
                        ("font-weight", "bold"), 
                        ("border", "2px solid black"), 
                        ("padding", "8px"), 
                        ("background-color", "#FFDAB9"),  # Fondo azul para la cabecera
                        ("color", "#4E342E"),  # Texto blanco
        ...
        if 'combos_seleccionados' in st.session_state and not st.session_state.combos_seleccionados.empty:
            ...
            if resumen:
                ...
                with col1:
                    ...
                    st.pyplot(fig)
                with col2:
                    # Ordenar los datos por ganancia estimada de mayor a menor
...
# Lógica de navegación basada en el estado de sesión
